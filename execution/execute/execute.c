...
static void	start_execute(t_tokenizer *tokenizer, t_infos *infos)
{
	pid_t	pid;
	char	*path;
	int		status;

	if (check_builtings(tokenizer->commands, \
		&infos->envp_info->env, 0) == 1)
		return ;
	if (tokenizer->is_heredoc)
	{
		ft_heredoc_init(tokenizer->commands, infos->envp_info->env, tokenizer);
		return ;
	}
